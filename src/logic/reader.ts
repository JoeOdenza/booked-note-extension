...
// Captures the given tab (or the active one, if not passed) as a PDF, uploads it to Claude,
// and asks it to extract PAGE_FIELDS into JSON -- the Claude-mode counterpart to DOM-mode's
// CSS-selector extraction. Must run in a privileged context (the service worker) -- chrome.tabs
// and chrome.debugger, which this and pageToPdf need, aren't available inside a content script.
export async function extractPageDataWithClaude(
  tabId?: number,
): Promise<PageDataSchema> {
  const resolvedTabId = tabId ?? (await getActiveTabId());
  const base64 = await pageToPdf(resolvedTabId);
